                            ...
                            <div class="row">
                                <div class="col-md-6">
                                    <div class="card mb-3 w-100">
                                        ...
                                        <div class="mt-3 wajib">
                                            <label>HP</label>
                                            <input class="form-control numeric" value="<?php echo (!empty($user['phone'])) ? $user['phone'] : '' ?>" id="phone" name="phone" type="text" placeholder="+6289123456789">
                                        </div>
                                    ...
                                    <div class="card mb-3 w-100">
                                        ...
                                        <div class="card-body text-black">
                                            <div class="table-responsive">
                                                <form method="post" action="<?= base_url('cart/update_red'); ?>" id="update-form">
                                                <table class="table">
                                                    <tbody>
                                                        <tr>
                                                        ...
                                                        <tr>
															...
															<th>
																...
																<?php // echo $row['quantity']; ?> 
																	<input style="border: none;" id="uintTextBox" type="number" name="count<?= $index; ?>" min="1" size="1" max="<?php echo floor(($total - ($row['quantity']*$row['price']))/200000); ?>" value="<?= $row['quantity']; ?>" readonly></input> 
															...
															<?php } else { ?>
															<th>
																<a href="<?= base_url('cart/remove_one_red/' . $row['id'] . '?Ukuran=' . $row['size']);?>" class="btn bgmerahhati text-white btn-sm">-</a> 
																<?php // echo $row['quantity']; ?> 
																	<input style="border: none;" id="uintTextBox" type="number" name="count<?= $index; ?>" min="1" max="9" size="1" value="<?= $row['quantity']; ?>" readonly></input> 
																<a href="<?= base_url('cart/add_one_red/' . $row['id'] . '?Ukuran=' . $row['size']);?>" class="btn bgmerahhati text-white btn-sm">+</a>
															</th>
															<?php } ?>
															<th>
																<!-- start here size atas-->
																<?php if($row['kategori'] == 'clothing') { ?>
															    <input type="hidden" name="sizeBefore<?= $index; ?>" value=<?= $row['size'] ?>>
                                                ...
                                                <div class="card">
                                                    ...
                                                    <div class="card-footer">
                                                        ...
                                                        <button id="update-button" type="submit" class="btn btn-sm bgmerahhati text-white">Update Jumlah dan Size</button>
...
<!-- snap  -->
	...
	<!-- snap  -->
	<form id="payment-form-mrg" method="post" action="https://m1salesforce.com/thanks/payment_mrg">
		<input type="hidden" name="<?= $this->security->get_csrf_token_name(); ?>" value="<?= $this->security->get_csrf_hash(); ?>" required>
		<input type="hidden" name="result_type" id="result-type-mrg" value=""></div>
		<input type="hidden" name="result_data" id="result-data-mrg" value=""></div>
	</form>
...
// 		bundlingLSS();
    ...
    function bundlingMSD() {
		...
		var imageUrl = "https://merryrianashop.com/assets/img/merchandise/3c52842817c5bf95d103669f9ee480e2.jpg";
		console.log(msd);
		console.log(mdd);
		console.log(paket);
	...
	function bundlingLSS() {
		...
		var imageUrl = "https://merryrianashop.com/assets/img/merchandise/a3dcca5757a3b9b0dad83df3c0570c85.jpeg";
		console.log(msd);
		console.log(lss);
		console.log(paket);
	...
	function bundlingMRFK() {
		...
		var imageUrl = "https://merryrianashop.com/assets/img/merchandise/43b9327e156c6eee4a7cc2bffb1ce1bd.jpg";
		console.log(mrfk1);
		console.log(mrfk2);
		console.log(paket);
	...
	function bundlingPoster() {
		...
		var imageUrl = "https://merryrianashop.com/assets/img/merchandise/f5fc9ed751c3b8847e7d03f6190b0794.jpg";
		console.log(poster1);
		console.log(poster2);
		console.log(paket);
	...
	function swalertAmbil(title, text, confirm) {
		...
		title: title,
		text: text,
		imageUrl: 'https://merryrianashop.com/assets/img/merchandise/650a345cc8d004cbb62c05db21ada40d.jpeg',
		imageHeight: 400,
		showCancelButton: true,
		focusConfirm: false,
...
// 			stringProductDash += dataItems[i]['merch_name'] + ' - ' + dataItems[i]['quantity'] + ', ';
			...
			var productName;
			if(dataItems[i]['kategori'] == "clothing") {
			    productName = dataItems[i]['merch_name'] + ' ' + dataItems[i]['size'];
			} else {
			    productName = dataItems[i]['merch_name'];
			}
			var objectProduct = {
				id: dataItems[i]['merch_id'],
...
// 		if(dataCustomer.aff == "merryrianashop") {
// 			$.ajax({
...
// 		} else {
    ...
    // 			window.location.href = response;
				...
				$.ajax({
					...
					success: function(response) {
						console.log(response);    
						$.ajax({	// berfungsi untuk menghapus session cart dan user
							url: "<?php echo base_url('cart/clear'); ?>",
							type: "GET",
							success: function(res) {
								console.log("atas");
								// window.location.href = link;	// redirect ke link pembayaran
							}
						})
					},
					error: function(e) {
