...
<body>
		...
		<div class="layout-w">
			...
			<div class="desktop-menu menu-top-w menu-activated-on-hover">
				<div class="menu-top-i">
					<div class="logo-w">
						<a class="logo" href="<?= base_url('online_program_ican_upgrade'); ?>"><img src="<?php echo base_url(); ?>assets/logo.png" style="width:50px;"></a>
					</div>
					...
					<div class="logged-user-w">
						<ul class="main-menu">
							...
							<!--</li>-->
							<li class="menu float-left" style="color:#fa163f; border-right: none;">
								<a href="<?php echo base_url('online_program_ican_upgrade'); ?>">
									<div class="icon-w">
							...
							<li class="menu float-left" style="color:#fa163f; border-right: none;">
								<a href="<?php echo base_url('cartprogramicanupgrade/list_chart'); ?>">
									<div class="icon-w">
											...
											<div class="col-md-12">
												<div class="card mb-3 w-100">
													<div class="card-header bgmerahhati text-white">
														Parents Detail
														<a href="<?php echo base_url('cartprogramicanupgrade/reset'); ?>" class="btn btn-light btn-sm text-uppercase text-dark float-right">Reset Form</a>
													</div>
												...
												<div class="card mb-3 w-100">
													...
													<div class="card-body text-black">
														<div class="row">
															<div class="col-md-6">
																...
																<div class="mt-3 wajib">
																	...
																	<select name="kurir" id="kurir" class="form-control" required>
																		<option value="0" selected disabled>Kurir Pilihan</option>
															...
															<div class="col-md-6">
																<!--start here dest code-->
																<div class="mt-3 wajib" id="kurir_jnt" style="display:none">
																...
																<div class="mt-3 wajib" id="kurir_jne">
										...
										<div class="row">
											<div class="col-md-12">
												<div class="card mb-3 w-100">
													<div class="card-header bgmerahhati text-white">
														Pesanan Anda
														<a href="<?php echo base_url('cartprogramicanupgrade/list_chart'); ?>" class="btn btn-light btn-sm text-uppercase text-dark float-right">Edit Keranjang</a>
													</div>
													<div class="card-body text-black">
																	...
																	?>
																			<tr>
																				...
																				<th scope="col">
																					...
																					<a href="<?= base_url('cartprogramicanupgrade/remove_one/' . $row['id']); ?>" class="btn bgmerahhati text-white btn-sm">-</a>
																					<?= $row['quantity'] ?>
																					<a href="<?= base_url('cartprogramicanupgrade/add_one/' . $row['id']); ?>" class="btn bgmerahhati text-white btn-sm">+</a>
																				</th>
																				<th scope="col">Rp. <?= number_format($row['price'] * $row['quantity']) ?></th>
																				<th scope="col"><a href="<?php echo site_url('cartprogramicanupgrade/remove/' . $row['id']); ?>" style="color: red;">Hapus Program</a></th>
																			</tr>
																	<?php }
																	} ?>
																	<tr>
	...
	<script type="text/javascript">
		...
		// });
		var input = document.querySelector("#phoneayah");
		window.intlTelInput(input, {
			initialCountry: "id",
			geoIpLookup: function(callback) {
				$.get('http://ipinfo.io', function() {}, "jsonp").always(function(resp) {
		...
		var input = document.querySelector("#phoneibu");
		window.intlTelInput(input, {
			initialCountry: "id",
			geoIpLookup: function(callback) {
				$.get('http://ipinfo.io', function() {}, "jsonp").always(function(resp) {
		...
		function swalertAmbil(title, text, confirm) {
			Swal.fire({
				...
				if (result.value) {
					...
					window.location.href = "<?php echo base_url('cartprogramicanupgrade/buy/23'); ?>";
				} else {
					console.log('gak jadi');
				}
			});
		...
		function swalertTawar(title, text, confirm) {
			Swal.fire({
				...
				if (result.value) {
					console.log(result);
					console.log(result.value);
					window.location.href = "<?php echo base_url('online_program_ican_upgrade'); ?>";
				} else {
					console.log('gak jadi');
				}
			});
		...
		function tambah_peserta(nama_program) {
			...
			var form_anak_baru = `<div class="col-md-6 mt-3" id="head` + jumlah_anak + `">
																...
																<div class="wajib">
																	<label for="">No HP anak<small><span style="color:red;"> *required</span></small></label>
																	<input class="form-control numeric" value="" id="nohpanak" name="nohpanak[` + jumlah_anak + `]" type="text" pattern="/^-?\d+\.?\d*$/" onKeyPress="if(this.value.length==12) return false;" placeholder="contoh : 8123456789" style="width:100%" required>
																</div>


																<div class="mt-3 wajib">
																	<label for="">Email<small><span style="color:red;"> *required</span></small></label>
																	<input class="form-control" value="" id="emailanak" name="emailanak[` + jumlah_anak + `]" type="text" placeholder="" required>
																</div>
																<small style="color: red;">Penting! Pastikan email ini AKTIF. Link kelas online akan di kirimkan ke email ini. 1 user = 1 email account. Email TIDAK BOLEH SAMA dengan peserta lain yang Anda daftarkan.</small>
															</div>`;
...
// 			});
			...
			$.ajax({
				...
				success: function(response) {
					...
					$.ajax({
						url: "<?php echo base_url('cartprogramicanupgrade/clear'); ?>",
						type: "GET",
						success: function(res) {
							console.log("Clear");
							window.location.href = response;
